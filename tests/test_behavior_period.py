...
def test_period_science_is_pooled_walk_forward_not_daily_average(tmp_path: Path) -> None:
    ...
    assert (out / "GEOMETRY.md").is_file()
    geometry = (out / "GEOMETRY.md").read_text(encoding="utf-8").lower()
    assert "removable" in geometry
    assert "timeframe" in geometry
...
def test_period_helpers_not_reexported_from_package_init() -> None:
    ...
    assert "run_geometry_rr" not in nq.research.__all__
