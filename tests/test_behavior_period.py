...
def test_period_science_is_pooled_walk_forward_not_daily_average(tmp_path: Path) -> None:
    ...
    assert (out / "VOLATILITY.md").is_file()
    assert "removable" in (out / "VOLATILITY.md").read_text(encoding="utf-8").lower()
...
def test_period_helpers_not_reexported_from_package_init() -> None:
    ...
    assert "run_volatility_target" not in nq.research.__all__
