...
def test_balanced_window_rotates_in_value() -> None:
    ...
    # أول برميل يؤسس VA فقط؛ لا يقيّم نفسه داخل حدود صنعها حجمه الحالي.
    assert states["decision_vah"].to_list()[0] is None
    assert states["in_value_fraction"].to_list()[0] == 0.0
    assert states["is_balanced"].to_list()[0] is False
...
def test_balance_flips_to_imbalance() -> None:
    # ثلاث نوافذ تدوير تؤكد التوازن، ثم نافذة اتجاهية تقلبه إلى اختلال.
    balanced = [("T", "B", 100 + d, 2, 0) for d in (0, 0, 1, -1, 0)]
    trend = [("T", "B", 100 + j, 2, 0) for j in range(10)]
    events = balanced * 3 + trend
    ts = (
        list(range(len(balanced)))
        + list(range(50, 50 + len(balanced)))
        + list(range(100, 100 + len(balanced)))
        + list(range(150, 150 + len(trend)))
    )
    ...
    assert states["decision_vah"].to_list()[1] == states["vah"].to_list()[0]
    assert states["is_balanced"].to_list()[2] is True
    assert states["is_balanced"].to_list()[3] is False
    assert states["flip_to_imbalance"].to_list()[3] is True
