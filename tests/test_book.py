...
def test_fill_partial_then_full() -> None:
    book = OrderBook()
    book.apply("A", "A", 102, 4, 1)
    book.apply("F", "N", 0, 1, 1)  # partial fill 1
    assert book.best_ask() == (102, 3)
    book.apply("F", "N", 0, 3, 1)  # remaining filled
