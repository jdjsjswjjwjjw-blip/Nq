...
def test_databento_synthesized_sequence_uses_source_metadata_not_row_order() -> None:
    rows = {
        "ts_event": [100, 100],
        "ts_recv": [102, 101],
        "publisher_id": [7, 7],
        ...
        "action": ["A", "A"],
        "side": ["A", "B"],
        "price": [20_001_000_000, 20_000_000_000],
        "size": [2, 1],
        "order_id": [20, 10],
    }
    original = pl.DataFrame(rows)
    reversed_rows = original.reverse()

    first = normalize_databento_frame(original).sort("sequence")
    second = normalize_databento_frame(reversed_rows).sort("sequence")

    assert first["order_id"].to_list() == [10, 20]
    assert second["order_id"].to_list() == [10, 20]
