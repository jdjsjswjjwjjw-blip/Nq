...
def test_snapshot_requires_instrument_scope_for_ambiguous_feature_names() -> None:
    first = FeatureStore().ingest_wide(
        pl.DataFrame({"bucket_start": [0], "availability_ts": [10], "delta": [1.0]}),
        value_columns=["delta"],
        version="v1",
        instrument_id=1,
    )
    second = wide_to_features(
        pl.DataFrame({"bucket_start": [0], "availability_ts": [10], "delta": [99.0]}),
        value_columns=["delta"],
        version="v1",
        instrument_id=2,
    )
    ...
    with pytest.raises(ValueError, match="instrument_id"):
        first.snapshot_series()
    with pytest.raises(ValueError, match="instrument_id"):
        first.point_in_time_join(pl.DataFrame({"t": [20]}), ts_col="t")

    scoped = first.snapshot_series(instrument_id=1)
    assert scoped["delta"].to_list() == [1.0]
