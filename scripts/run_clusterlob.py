#!/usr/bin/env python3
"""ClusterLOB: دفتر يومي → k-means على القطار → OFI عنقودي مقابل مُجمَّع.

التسميات من ``period_*/fold_scores.parquet``. MBO من IDrive يوماً واحداً ثم يُسقط.
لا holdout أيلول–كانون. لا لصق تدفق خام.

    .venv/bin/python scripts/run_clusterlob.py \\
      --days-root data/runs/auction_behavior_year \\
      --mbo-root /opt/IDriveForLinux/.../Restore_Data/2025 \\
      --output data/runs/auction_behavior_year/clusterlob
"""
...
from nq.research.clusterlob import (  # noqa: E402
    HOLDOUT_START_DATE,
    run_clusterlob,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="ClusterLOB clustered OFI vs pooled OFI on y_phase_extend"
    )
    ...
    scored, diagnostics = run_clusterlob(days, seed=args.seed, holdout_start=args.holdout_start)
    diagnostics["mbo_root"] = None if args.mbo_root is None else str(args.mbo_root)
    diagnostics["max_days"] = args.max_days
    written = write_clusterlob_report(scored, diagnostics, args.output)
    print((written / "CLUSTERLOB.md").read_text(encoding="utf-8"))
