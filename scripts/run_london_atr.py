#!/usr/bin/env python3
"""طبقة ز: هدف/وقف ATR لندن + هاي آسيا على OOF — ليست أداة تنفيذ حيّة.

  .venv/bin/python scripts/run_london_atr.py \\
...
from nq.research.london_atr_target import (  # noqa: E402
    LondonAtrConfig,
    run_london_atr,
    run_london_atr_grid,
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="OOF test: London-session ATR target/stop (not live execution)"
    )
    ...
    parser.add_argument("--min-atr-pts", type=float, default=30.0)
    parser.add_argument("--target-atr-frac", type=float, default=0.5)
    ...
    parser.add_argument(
        "--grid",
        action="store_true",
        help="asia on/off × min_rr 2.0/1.5 × target frac 0.5/0.4 (pre-declared)",
    )
    ...
    written = write_london_atr_report(report, args.output)
    if args.grid:
        grid = run_london_atr_grid(
            labeled,
            blended,
            config=cfg,
            oof_availability_ts=oof_ts,
        ...
        if grid.height:
            grid.write_parquet(written / "london_atr_grid.parquet")
            (written / "london_atr_grid.json").write_text(
    ...
    log.done(
        f"layer=london_atr_target traded={report.diagnostics.get('n_traded')} "
        f"skipped={report.diagnostics.get('n_skipped')}"
    )
    print(f"outputs: {written.resolve()}/", flush=True)
    print((written / "LONDON_ATR.md").read_text(encoding="utf-8"), flush=True)
