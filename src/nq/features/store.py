...
class FeatureStore:
    ...
    @staticmethod
    def _reject_ambiguous_instrument_scope(
        scoped: pl.DataFrame,
        *,
        instrument_id: int | None,
    ) -> None:
        if instrument_id is not None or scoped.height == 0:
            return
        ambiguous = (
            scoped.group_by(FEATURE)
            .agg(pl.col(INSTRUMENT_ID).n_unique().alias("n_instruments"))
            .filter(pl.col("n_instruments") > 1)
        )
        if ambiguous.height:
            names = ambiguous[FEATURE].head(5).to_list()
            raise ValueError(
                "instrument_id is required when feature names are present for multiple "
                f"instruments; ambiguous features={names}"
            )
    ...
    def snapshot_series(
        ...
        *,
        ...
        self._reject_ambiguous_instrument_scope(scoped, instrument_id=instrument_id)
        wide = scoped.pivot(
            on=FEATURE,
