...
_FAIL_FVG_NQ_ONLY_FOCUS = ("fail_fvg", "nq_delta")
...
def run_fail_fvg_research(
    ...
    *,
    ...
    cfg = PipelineConfig(
        ...
        signal_columns=_FAIL_FVG_NQ_ONLY_FOCUS if mnq is None else _FAIL_FVG_FOCUS,
        quiet=quiet,
    )
    partner = mnq if mnq is not None else nq
    focus = _FAIL_FVG_NQ_ONLY_FOCUS if mnq is None else _FAIL_FVG_FOCUS
    ...
    return FailFvgResearchResult.from_unified(result, signal_columns=focus)
