...
_FAIL_BREAKOUT_NQ_ONLY_FOCUS = tuple(
    c for c in _FAIL_BREAKOUT_FOCUS if c not in {"trap_setup", "mnq_delta", "lead_lag"}
)
...
def run_fail_breakout_research(
    ...
    *,
    ...
    cfg = PipelineConfig(
        ...
        signal_columns=_FAIL_BREAKOUT_NQ_ONLY_FOCUS if mnq is None else _FAIL_BREAKOUT_FOCUS,
        quiet=quiet,
    )
    partner = mnq if mnq is not None else nq
    focus = _FAIL_BREAKOUT_NQ_ONLY_FOCUS if mnq is None else _FAIL_BREAKOUT_FOCUS
    ...
    return FailBreakoutResearchResult.from_unified(result, signal_columns=focus)
