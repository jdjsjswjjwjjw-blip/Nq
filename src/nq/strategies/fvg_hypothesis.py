    ...
    *,
    ...
    for fold_i, fold in enumerate(folds):
        ...
        for col_i, col in enumerate(cols, start=1):
            ...
            if (
                best_ic is None
                or abs(ic) > abs(best_ic)
                or (abs(ic) == abs(best_ic) and ic > best_ic)
            ):
                best_ic = ic
                best_name = col
            if log is not None:
        ...
        rows.append(
            {
                "fold": fold_i,
                "selected": best_name,
                "train_ic": float(0.0 if best_ic is None else best_ic),
        ...
        if log is not None:
            log.op(  # type: ignore[union-attr]
                ...
                f"train_ic={(0.0 if best_ic is None else best_ic):.4g} · "
