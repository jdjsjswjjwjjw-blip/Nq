...
def _update_session_scoped_state(
    state: _SessionScopedState,
    *,
    mnq_signed: int,
    nq_high: float,
    nq_low: float,
    ...
    state.mnq_signed = mnq_signed
    state.nq_high = nq_high
    state.nq_low = nq_low
    state.mnq_high = mnq_high
    state.mnq_low = mnq_low
    state.prev_nq_mid = prev_nq_mid
...
def _trap_setup(
    ...
    *,
    ...
    """إعداد مصيدة سببي: MNQ يكسر حدًا سابقًا دون تأكيد NQ."""
    if mnq_delta >= min_delta and mnq_high > 0 and mnq_mid > mnq_high and nq_mid <= nq_high:
        return 1.0
    if mnq_delta <= -min_delta and mnq_low > 0 and mnq_mid < mnq_low and nq_mid >= nq_low:
        return -1.0
    return 0.0
...
def _tick_row(
    *,
    ...
    if nq_mid > 0:
        ...
        nq_low = nq_mid if nq_low == 0.0 else min(nq_low, nq_mid)
    ...
    return row, mnq_signed, nq_high, nq_low, mnq_high, mnq_low, next_prev_nq_mid
...
def build_tick_stream(
    ...
    *,
    ...
    for i, (action, side, price, size, order_id, ts, seq, inst, contract_id) in enumerate(
        ...
        session_id = trading_session_id_from_ns(int(ts))
        ...
        row, mnq_signed, nq_high, nq_low, mnq_high, mnq_low, prev_nq_mid = _tick_row(
            ...
            nq_book=nq_book,
            mnq_book=mnq_book,
            nq_profile=session_state.profile,
            regime_tracker=session_state.regime_tracker,
            mnq_signed=session_state.mnq_signed,
            nq_high=session_state.nq_high,
            nq_low=session_state.nq_low,
            mnq_high=session_state.mnq_high,
