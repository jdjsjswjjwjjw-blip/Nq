...
from nq.statistics.resampling import TestResult, block_permutation, permutation_test
...
def _default_block_permutation_size(n: int) -> int:
    return max(1, min(n, int(np.rint(np.sqrt(n)))))
...
def measure_mfig(
    ...
    *,
    ...
    block_size = _default_block_permutation_size(returns.shape[0])
    for i in range(n_permutations):
        perm = block_permutation(returns, block_size=block_size, rng=generator)
...
def measure_cer(
    ...
    *,
    ...
    for block_name, cols in resolved.items():
        ...
        block_size = _default_block_permutation_size(pooled.shape[0])
        for i in range(n_permutations):
            perm = block_permutation(pooled, block_size=block_size, rng=generator)
