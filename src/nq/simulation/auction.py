...
from nq.core.session import TRADING_SESSION_ID
...
def auction_states(
    ...
    *,
    ...
    prev_range = price_range.shift(1).over(TRADING_SESSION_ID)
    prev_high = pl.col("high").shift(1).over(TRADING_SESSION_ID)
    prev_low = pl.col("low").shift(1).over(TRADING_SESSION_ID)
    in_value_fraction = (
    ...
    * ``vp_flip_to_imbalance`` — انتقال من توازن → اختلال.
    ...
    return (
        ...
        .with_columns(
            ...
            (
                pl.col("is_balanced").shift(1).over(TRADING_SESSION_ID).fill_null(value=False)
                & ~pl.col("is_balanced")
            )
            .cast(pl.Float64)
