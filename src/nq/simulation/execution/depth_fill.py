...
def execution_forward_returns_depth(
    ...
    *,
    ...
    for t in range(max(0, n - latency_steps - horizon)):
        ...
        if entry_long is None and fa is not None and np.isfinite(fa[entry_t]):
            entry_long = float(fa[entry_t]) + slip
        if exit_long is None and fb is not None and np.isfinite(fb[exit_t]):
            exit_long = float(fb[exit_t]) - slip
        if entry_short is None and fb is not None and np.isfinite(fb[entry_t]):
            entry_short = float(fb[entry_t]) - slip
        if exit_short is None and fa is not None and np.isfinite(fa[exit_t]):
            exit_short = float(fa[exit_t]) + slip
