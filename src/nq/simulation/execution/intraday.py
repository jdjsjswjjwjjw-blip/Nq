...
def realistic_execution_forward_returns(
    ...
    *,
    ...
    entry_idx = decisions + latency_steps
    exit_idx = entry_idx + horizon
    entry_long = a[entry_idx] + slip
    exit_long = b[exit_idx] - slip
    entry_short = b[entry_idx] - slip
    exit_short = a[exit_idx] + slip

    with np.errstate(divide="ignore", invalid="ignore"):
        long_fwd[decisions] = np.where(
            entry_long > 0,
            (exit_long - entry_long) / entry_long - comm,
            np.nan,
        )
        short_fwd[decisions] = np.where(
            entry_short > 0,
            (entry_short - exit_short) / entry_short - comm,
            np.nan,
        )
    return long_fwd, short_fwd
