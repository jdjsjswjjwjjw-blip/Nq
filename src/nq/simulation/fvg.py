    ...
    *,
    ...
    if m30.height == 0:
        if progress is not None:
            progress.op("failed_fvg_from_bars: تخطّي — لا شموع إشارة")
    ...
    n_rows = m30.height
    if progress is not None:
        progress.op(f"failed_fvg_from_bars: مسح {n_rows:,} شمعة · fvgs={len(fvg_rows)}")

    for i, row in enumerate(m30.iter_rows(named=True), start=1):
        if progress is not None:
            progress.heartbeat(i, n_rows, label="fvg_bars")
...
def failed_fvg_features(
    ...
    *,
    ...
    if progress is not None:
        progress.op(f"OHLCV جاهز: h1={h1.height:,} · signal={signal_bars.height:,}")
