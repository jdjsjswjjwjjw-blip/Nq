...
def developing_value_area(
    ...
    *,
    ...
    per_price = (
        trades.group_by([BUCKET_START, "price"])
        .agg(
    ...
    profiles: dict[str, DevelopingVolumeProfile] = {}
    previous_poc: dict[str, int] = {}
    for (bucket_start,), group in per_price.group_by([BUCKET_START], maintain_order=True):
        bucket_end = int(group[BUCKET_END][0])
        session_id = trading_session_id_from_ns(bucket_end)
        profile = profiles.setdefault(session_id, DevelopingVolumeProfile(fraction=fraction))
        for price, volume in zip(
            group["price"].to_list(),
            group["volume"].to_list(),
            strict=True,
        ):
            profile.add_trade(int(price), int(volume))
        va = profile.value_area()
        if va is None:  # pragma: no cover - group is always non-empty here
            continue
        migration = 0 if session_id not in previous_poc else va.poc - previous_poc[session_id]
        previous_poc[session_id] = va.poc
        rows.append(
            {
    ...
    return result.with_columns(pl.col(BUCKET_END).alias(AVAILABILITY_TS)).select(
        BUCKET_START,
        TRADING_SESSION_ID,
        "poc",
        "vah",
