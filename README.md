...
كسر فاشل (Failed Breakout) من MBO → شموع سببية **بتركيز فوليوم**:
...
* **إصلاح تسريب الدخول:** الإشارة عند إغلاق الشمعة؛ `fb_entry_ref = close`؛  
  `fb_break_level` تحليلي فقط — التقييم عبر مسار الألفا (bid/ask + slippage).
* التنخيل: walk-forward purged + تعزيزات SSL/سياق (ليس إعادة كتابة القاعدة).
